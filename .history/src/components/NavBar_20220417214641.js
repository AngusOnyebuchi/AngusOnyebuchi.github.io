...
export default function NavBar() {
    ...
    return(
        <div className="sm:fixed bg-[#0a192f] text-white w-full p-8 flex justify-between items-center border-b-2 border-pink-300">
            ...
            <ul className={nav ? 'hidden' : 'fixed absolute top-0 left-0 w-full h-full bg-inherit flex flex-col justify-center items-center my-8 sm:hidden'}>
                    <li onClick={toggle} className="py-6 text-4xl"><Link to="home" smooth={true} duration={500} >Home</Link></li>
                    <li className="py-6 text-4xl"><Link to="education" smooth={true} duration={500} >Education</Link></li>
                    <li className="py-6 text-4xl"><Link to="experience" smooth={true} duration={500} >Experience</Link></li>
