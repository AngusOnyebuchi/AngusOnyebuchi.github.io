import React from "react";

export default function Home() {
    return (
        <div className="w-full h-screen p-4">
            <h1 className="text-4xl md:text-6xl font-bold uppercase pt-24 text-center text-blue-900">Maduabuchi Angus Modum</h1>
            <p className="font-bold text-center text-xl md:text-2xl max-w-[1000px] mx-12 mt-4">Aim: Striving to be the best, obtain hands-on-deck skills, 
                to work tirelessly towards the growth, development and success of the organization and to make sure my impact 
                as a professional is felt positively in the day to day activities of the organization.</p>

            <div className="grid grid-cols-3 gap-4 text-2xl text-center">
                <h4 className="mx-0">Lagos, Nigeria</h4>
                <h4 className="mx-0">+2349037872259</h4>
                <h4  className="mx-0">Angusonyebuchi800@gmail.com</h4>
            </div>        
        </div>
    )
} 